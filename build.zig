...
pub fn build(b: *std.Build) void {
    ...
    // Zig 0.17: OptimizeMode tags are lowercase (.debug/.fast/…); CLI still accepts
    // legacy -Doptimize=Debug|ReleaseFast via std.lang.Optimize.fromString.
    ...
    const package_target_override = b.option(PackageTarget, "package-target", "Package target: macos, windows, linux (default: selected native platform; null platform is not packageable)");
    ...
    // Package CLI still wants legacy names (Debug/ReleaseFast) for artifact paths.
    const package_optimize_name = optimizeCliName(package_optimize);
    ...
    // Zig 0.17 removed Build.sysroot; keep a local for macOS host/framework flags only.
    const macos_sysroot: ?[]const u8 = if (target.result.os.tag == .macos) macosSdkPath(b) else null;
    ...
    // Package target defaults to the selected native platform (macos/linux/windows).
    ...
    const runner_mod = localModule(b, target, optimize, "desktop/runner.zig");
    ...
    const app_mod = localModule(b, target, optimize, "desktop/main.zig");
    ...
    linkPlatform(b, target, app_mod, exe, selected_platform, web_engine, web_layer, native_sdk_path, cef_dir, cef_auto_install, macos_sysroot);
    b.installArtifact(exe);

    // Reuse the existing SolidJS/Vite SPA at the repo root (pnpm, dist/).
    // Do not scaffold a separate frontend/ tree.
    const frontend_install = b.addSystemCommand(&.{ "pnpm", "install", "--frozen-lockfile" });
    const frontend_install_step = b.step("frontend-install", "Install frontend dependencies (pnpm)");
    ...
    const frontend_step = b.step("frontend-build", "Build the SolidJS frontend into dist/");
    ...
    const package_exe = if (package_optimize == optimize) exe else pkg: {
        ...
        const package_runner_mod = localModule(b, target, package_optimize, "desktop/runner.zig");
        ...
        const package_app_mod = localModule(b, target, package_optimize, "desktop/main.zig");
        ...
        linkPlatform(b, target, package_app_mod, built, selected_platform, web_engine, web_layer, native_sdk_path, cef_dir, cef_auto_install, macos_sysroot);
        break :pkg built;
    };
    ...
    if (package_target) |pkg_target| {
        ...
        const package = b.addSystemCommand(&.{
            ...
            // Signing remains none: local/unsigned package only (no installer).
            "--signing",
            "none",
            "--output",
            b.fmt("zig-out/package/{s}-{s}-{s}-{s}{s}", .{ app_exe_name, package_version, @tagName(pkg_target), package_optimize_name, packageSuffix(pkg_target) }),
            "--binary",
        });
        // The CLI resolves SDK-owned package inputs (the vendored WebView2
        // loader) from the framework root; pin NATIVE_SDK_PATH to the same
        // tree this build linked against.
        ...
        // -Dplatform=null with no -Dpackage-target: keep configure green for
        // diagnostic host builds, but refuse the package step at run time.
...
// so this only changes Debug builds. Retained under Zig 0.17 until
// re-measured on the pinned toolchain.
fn useLlvmWorkaround(target: std.Build.ResolvedTarget) ?bool {
    return if (target.result.cpu.arch == .x86_64) true else null;
}

// Zig 0.17 removed Build.pathFromRoot; join against the package build root.
fn pathFromRoot(b: *std.Build, sub_path: []const u8) []const u8 {
    return b.root.joinString(b.graph.arena, sub_path) catch @panic("OOM");
}

// Native package CLI and artifact paths still use legacy optimize names.
fn optimizeCliName(mode: std.builtin.OptimizeMode) []const u8 {
    return switch (mode) {
        .debug => "Debug",
        .safe => "ReleaseSafe",
        .fast => "ReleaseFast",
...
// Zig 0.17 removed Step.Run.addPathDir; prepend to PATH (or WINEPATH is
// not needed for pure Windows targets run natively). Windows WebView2
// loader discovery only.
fn addPathDir(run: *std.Build.Step.Run, search_path: []const u8) void {
    const b = run.step.owner;
    const environ_map = run.getEnvMap();
    const key = "PATH";
    if (environ_map.get(key)) |pp| {
        const new_path = b.fmt("{s}{c}{s}", .{ pp, std.fs.path.delimiter, search_path });
...
// resolves through zig's graph.release_mode, and only when neither was
// passed does the role keep its own default — debug for the dev loop,
// fast for the exe `zig build package` wraps.
...
fn linkPlatform(b: *std.Build, target: std.Build.ResolvedTarget, app_mod: *std.Build.Module, exe: *std.Build.Step.Compile, platform: PlatformOption, web_engine: WebEngineOption, web_layer: bool, native_sdk_path: []const u8, cef_dir: []const u8, cef_auto_install: bool, macos_sysroot: ?[]const u8) void {
    if (platform == .macos) {
        switch (web_engine) {
            .system => {
                const sdk_include = if (macos_sysroot) |sysroot| b.fmt("-I{s}/usr/include", .{sysroot}) else "";
                const flags: []const []const u8 = if (macos_sysroot) |sysroot| &.{ "-fobjc-arc", "-fno-sanitize=builtin", "-ObjC", "-mmacosx-version-min=11.0", "-isysroot", sysroot, sdk_include } else &.{ "-fobjc-arc", "-fno-sanitize=builtin", "-ObjC", "-mmacosx-version-min=11.0" };
            ...
            .chromium => {
                ...
                const sdk_include = if (macos_sysroot) |sysroot| b.fmt("-isystem{s}/usr/include", .{sysroot}) else "";
                const flags: []const []const u8 = if (macos_sysroot) |sysroot| &.{ "-fobjc-arc", "-fno-sanitize=builtin", "-ObjC++", "-std=c++17", "-stdlib=libc++", "-mmacosx-version-min=11.0", "-isysroot", sysroot, sdk_include, include_arg, define_arg } else &.{ "-fobjc-arc", "-fno-sanitize=builtin", "-ObjC++", "-std=c++17", "-stdlib=libc++", "-mmacosx-version-min=11.0", include_arg, define_arg };
...
fn addWebView2RuntimeRunFiles(b: *std.Build, target: std.Build.ResolvedTarget, run: *std.Build.Step.Run, web_engine: WebEngineOption, web_layer: bool, native_sdk_path: []const u8) void {
    ...
    addPathDir(run, pathFromRoot(b, b.pathJoin(&.{ native_sdk_path, loader_dir })));
}
...
/// Default package target follows the selected native platform.
/// Returns null when the selected backend is not packageable (`null`).
fn defaultPackageTarget(selected: PlatformOption) ?PackageTarget {
...
/// Read `.version = "…"` from an app.zon source slice. Pure / embed-friendly.
/// Returns null when the field is missing or unterminated (caller supplies fallback).
fn packageVersionFromAppZon(source: []const u8) ?[]const u8 {
...
// Focused pure-helper coverage (no unavailable Build APIs). These run as part
// of `zig test` only when this file is a root module — kept as comptime checks
// so configure-time path defaults stay honest without a separate test harness.
comptime {
    const sample =
        \\.{
        \\    .name = "onyx",
    ...
    if (defaultPackageTarget(.windows) != .windows) @compileError("defaultPackageTarget(windows)");
    if (defaultPackageTarget(.null) != null) @compileError("defaultPackageTarget(null) must reject packaging");
}
