...
for an existing F-item rather than a new idea, and three original premises were falsified and
corrected during that pass. Read that section before treating any entry as novel.
...
`src/shell/memberWindow.ts`), a serious 24-file E2EE group stack (`src/lib/e2ee/`), hybrid
...
`src/lib/store/store.ts` is **18,389 lines** — the client's velocity ceiling (**GC-01**). And
...
| # | ID | Feature | Scope |
|---|----|---------|-------|
| 1 | GS-01 | server.zig Strangler (102,158 lines → modules) | Server |
| 2 | **GB-07** | **Live Captions last mile** — daemon side already built; client has no caption surface | **Both** |
| 3 | **GB-06** | **Raid Shield** — mesh-coordinated raid clamp + honest client banner | **Both** |
| 4 | **GC-01** | **store.ts Strangler** (18,389 lines → domain facades) | **Client** |
| 5 | **GB-03** | **Verifiable Transparency Ledger** — client-verifiable moderation proofs | **Both** |
| 6 | GS-03 | Class-based Admission Fabric | Server |
| 7 | GS-02 | Sketch Telemetry Plane | Server |
| 8 | **GB-14** | **Consent-Gated Unfurl Proxy** — client half already exists | **Both** |
| 9 | **GC-02** | **Extension Sandbox Host** — ship isolation before third-party code | **Client** |
| 10 | GS-11 | Link Supervision (circuit breakers on S2S) | Server |

**Seven of the top ten need client work.** The client is on the critical path for 0.7.
...
|----|---------|----|--------|---------------|
| GC-01 | **store.ts Strangler** — 18,389-line store → domain facades, reactivity unchanged | XL | **0.7 P0** | onyx-store, solidjs-coder |
...
| GC-09 | **Theme Studio** — user OKLCH themes with automatic AA enforcement | M | 0.7 P1 | onyx-theme, onyx-a11y, solidjs-coder |
...
|---|---|---|
| GC-01 | `src/lib/store/store.ts` (18,389 lines), `useStore` bridge | domain facades |
...
| GC-09 | OKLCH factory in `src/lib/theme/`, `src/lib/color/`, `AppearancePanel.tsx` | studio + AA gate |
| GC-10 | `src/lib/intelligence/translateMessage.ts`, `provenance.ts`, `AiPolicyBadge.tsx` | the surface |
| GC-11 | 24 files in `src/lib/e2ee/` (`groupTrustStore`, `keyPinning`, …) | one inspectable panel |
...
| GB-03 | **Transparency Ledger** — client-verifiable moderation proofs | new `src/lib/ledger/` + GC-11 tab | L | 0.7 P1 |
...
| GB-07 | **Live Captions** — server side already built | new `src/lib/cadence-media/captions.ts`, `CaptionsOverlay.tsx` | M | **0.7 P0** |
...
| GB-10 | **Semantic Mesh Search** — network-wide search fused with local | `src/lib/vault/searchVaultHybrid.ts`, `src/lib/search/rankingBoost.ts` | L | post-0.7 |
...
GS-16 qlog Observability Export · GS-17 Durable Monotonic IDs · GS-18 Mesh ACL Filters ·
...
2. **0.7 P0:** GC-02 (sandbox before ecosystem), GC-08 (a11y — also the credibility floor for
   GB-07), then the client halves of GB-07, GB-14, GB-06.
3. **0.7 P1 surfaces:** GC-03, GC-06, GC-11, GC-13, GC-09, GC-10, GC-14, GC-04, GC-07, GC-15, GC-05.
4. **post-0.7:** GC-12 (blocked on GC-01), then the client halves of GB-01, GB-05, GB-09, GB-10, GB-15.

## Verdict for the client half

**GO** for GC-01, GC-02, GC-08 and the client halves of GB-07, GB-14, GB-06 — no unresolved
...
- **CONFIRMED** — every "already exists" claim above, verified by reading paths and line counts at
  HEAD (not from memory). This includes the two load-bearing negatives: `src/lib/extensions/` has
  **no** isolation host, and there is **no** caption surface for the daemon's existing
  `MEDIA CAPTION` frames.
