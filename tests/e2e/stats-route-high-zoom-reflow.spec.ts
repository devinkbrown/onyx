...
test('keeps the PublicFrame Stats route usable at 400% zoom', async ({ page }) => {
  ...
  const main = frame.getByRole('main', { name: 'Onyx network stats' });
  ...
  const observation = main.locator('.stats-observation');
  const sections = main.getByRole('navigation', { name: 'Stats sections' });
  ...
  await expect(header.locator('a[href="/stats/"]')).toHaveCount(0);
  await expect(main.getByRole('heading', { name: 'The rooms in motion' })).toBeVisible();
  await expect(observation).toHaveCount(1);
  await expect(observation).toHaveAttribute('role', 'status');
  await expect(observation).toHaveAttribute('aria-live', 'polite');
  await expect(observation).toHaveAttribute('aria-atomic', 'true');
  await expect(observation).toContainText(/stats (?:current|stale|time mismatch|undated|incomplete|unavailable)/);
  ...
  const geometry = await page.evaluate(() => {
    ...
    return {
      ...
      routeActions: Array.from(document.querySelectorAll<HTMLElement>(
        '.stats-page .stats-view-nav a, .stats-page .stats-refresh, .stats-page .r-btn, .stats-page .data-action, .stats-page .stats-control-group button, .stats-page .stats-room-search input',
      )).filter(visible).map(bounds),
