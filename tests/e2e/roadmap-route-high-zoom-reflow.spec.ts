...
test('keeps the PublicFrame Roadmap route usable at 400% zoom', async ({ page }) => {
  ...
  const main = frame.getByRole('main', { name: 'Onyx product roadmap' });
  ...
  await expect(footer.locator('a[href="/roadmap/"]')).toHaveCount(0);
  ...
  await expect(main.getByRole('heading', { name: /rooms, calls, catch-up, and a Home Screen/i })).toBeVisible();
  await expect(main.getByRole('heading', { name: /rooms that stay open/i })).toBeVisible();
