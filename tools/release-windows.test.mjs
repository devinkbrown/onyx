...
describe('release-windows naming', () => {
  it('matches build.zig package dir and release zip basenames', () => {
    expect(packageDirName()).toBe('onyx-0.1.3-windows-ReleaseFast');
    expect(releaseZipBaseName()).toBe('onyx-0.1.3-windows-x86_64-ReleaseFast-unsigned');
    const paths = resolveReleasePaths('/repo', '0.1.3');
    expect(paths.packageDir).toBe('/repo/zig-out/package/onyx-0.1.3-windows-ReleaseFast');
    expect(paths.zipPath).toMatch(/onyx-0\.1\.3-windows-x86_64-ReleaseFast-unsigned\.zip$/);
  });
});
...
describe('release-windows zip + pipeline (fixture)', () => {
  it('zips a fixture package and writes checksums fail-closed', () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        '.{ .target = "windows", .version = "0.1.3", .optimize = "ReleaseFast", .signing = "none", .subsystem = "gui" }\n',
      );
  ...
  it('runWindowsRelease skips zig when --skip-package-build and validates fixture', async () => {
    ...
    try {
      ...
      const pkg = join(dir, 'zig-out/package/onyx-0.1.3-windows-ReleaseFast');
