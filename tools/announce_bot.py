...
def module_count() -> str:
    """Count UI components (.tsx files), excluding deps/build output."""
    try:
        return subprocess.check_output(
            ["bash", "-lc",
             f"find {REPO} -name '*.tsx' | grep -vF -e /node_modules/ -e /.next/ "
...
def test_count() -> str:
    ...
    try:
        return subprocess.check_output(
            ["bash", "-lc",
             f"{{ find {REPO} -name '*.test.ts'; find {REPO} -name '*.test.tsx'; "
             f"find {REPO} -name '*.spec.ts'; find {REPO} -name '*.spec.tsx'; }} | "
...
def loc_count() -> str:
    try:
        out = subprocess.check_output(
            ["bash", "-lc",
             f"{{ find {REPO} -name '*.ts'; find {REPO} -name '*.tsx'; }} | "
...
def stats_lines() -> list[str]:
    return [
        f"{B}{CYAN}build{RST} :: components={GREEN}{module_count()}{RST} "
        f"tests={GREEN}{test_count()}{RST} loc={GREEN}{loc_count()}{RST} "
        f"commits={GREEN}{git('rev-list','--count','HEAD')}{RST} Solid+Vite",
...
HELP = (
    "commands: !project !stats !tests !components !commit !changelog !diff !progress "
...
class Bot:
    ...
    def commit_lines(self, rev: str, tc: str, with_delta: bool) -> list[str]:
        ...
        lines = [
            ...
            f" {GREY}components{RST} {B}{CYAN}{module_count()}{RST}"
    ...
    def command(self, nick: str, reply: str, text: str) -> None:
        ...
        if cmd == "help":
            ...
            self.msg(reply, f"components: {GREEN}{module_count()}{RST} .tsx files")
