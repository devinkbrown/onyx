...
import type { OpcodecWasm, OpvisEncoder } from './OpcodecWasm';

// Covers the encoder fallback ladder added so worker video encoding keeps
// working even when the opvis WASM codec rejects the requested resolution
// (it returns a null handle for some sizes, e.g. 1920x1080, making
// OpvisEncoder throw "opvis encoder init failed").
...
function fakeWasm(accepted: ReadonlyArray<[number, number]>) {
  ...
  const videoEncoder = vi.fn((width: number, height: number) => {
    if (!ok.has(`${width}x${height}`)) {
      throw new Error('opvis encoder init failed');
    }
    return { width, height, destroy: vi.fn() } as unknown as OpvisEncoder;
  });
...
describe('buildEncoder fallback ladder', () => {
  ...
  it('propagates the error when no size is accepted', () => {
    const { wasm } = fakeWasm([]);
    expect(() => buildEncoder(wasm, 0, 1920, 1080, 70, 'camera', 60)).toThrow(/opvis encoder init failed/);
  });
});
