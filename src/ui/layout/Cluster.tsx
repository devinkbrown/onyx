...
export type ClusterProps = JSX.HTMLAttributes<HTMLElement> & {
  as?: LayoutAs;
  gap?: LayoutSpace;
  align?: LayoutAlign;
...
export function Cluster(props: ClusterProps) {
  ...
  const gap = createMemo(() => resolveSpace(local.gap, '2'));
  const align = createMemo(() => resolveAlign(local.align));
  const justify = createMemo(() => resolveJustify(local.justify));
  const className = createMemo(() => joinLayoutClass('ui-layout', 'ui-cluster', local.class));
