...
function _loadChannelNotify(): Map<string, 'all' | 'mentions' | 'none'> {
  ...
  try {
    ...
    const entries = Object.entries(JSON.parse(raw) as Record<string, string>)
      .filter((entry): entry is [string, 'mentions' | 'none'] =>
        entry[1] === 'mentions' || entry[1] === 'none',
      );
    return new Map(entries);
  } catch { return new Map(); }
}

function _saveChannelNotify(channelNotify: Map<string, 'all' | 'mentions' | 'none'>): void {
  if (typeof window === 'undefined') return;
  try {
    localStorage.setItem('ocean-channel-notify', JSON.stringify(Object.fromEntries(channelNotify)));
  } catch {}
}
