...
export default function DMPinsPanel() {
  ...
  return (
    <ModalShell
      ...
      kicker={`DM with ${nick}`}
      titleId="dmp-panel-title"
      closeLabel="Close pinned messages"
      headerExtra={pinned.length > 0 ? <span className="dmp-count">{pinned.length}</span> : undefined}
      flushBody
    >
      ...
      <style>{`
        ...
        .dmp-empty {
          ...
          gap: var(--sp-3, 12px); padding: var(--sp-10, 40px) var(--sp-6, 24px); text-align: center;
        }
        .dmp-empty-icon { font-size: 40px; }
        .dmp-empty-title {
          font-size: var(--text-lg, 16px); font-weight: 700; color: var(--text-primary);
          margin: 0;
        }
        .dmp-empty-hint {
        ...
        .dmp-card {
          ...
          transition: border-color var(--t-control, 150ms), border-left-color var(--t-control, 150ms), background var(--t-control, 150ms);
        }
        .dmp-card:hover {
        ...
        .dmp-card-nick {
          font-size: var(--text-sm, 13px); font-weight: 600; color: var(--text-primary);
        ...
        .dmp-unpin-btn {
          ...
          opacity: 0; transition: opacity var(--t-control, 150ms), background var(--t-control, 150ms), color var(--t-control, 150ms);
          flex-shrink: 0;
        }
        .dmp-card:hover .dmp-unpin-btn,
        .dmp-unpin-btn:focus-visible { opacity: 1; }
        .dmp-unpin-btn:hover {
